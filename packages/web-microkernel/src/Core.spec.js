...
const serialize = async (exports, { reviver } = {}) => {
  ...
  const childProcessPromise = kernel.createProcess({})

  childProcess = core.ChildProcess(
    {
  ...
  const childProcessFunction = await childProcessPromise

  return new Promise(async (resolve, reject) => {
    childProcess.addEventListener(`error`, reject)
    kernel.addEventListener(`error`, reject)
  ...
  describe(`types`, () => {
    ...
    describe(`function`, () => {
      ...
      it(`allows functions only within exports`, async () => {
        ...
        const { kernel, value } = await serialize(compose)

        const errorPromise = new Promise(resolve => {
          kernel.addEventListener(`error`, resolve)
        })
...
xit(`shares capabilities between windows`, async () => {
  let childWindow
  let processName
  const rootWindow = MockWindow(`root`)
  const windows = new Map([[null, rootWindow]])
  ...
  const insertHTML = html => {
    processName = html.match(/name=\"(.+)\"/)[1]
    childWindow = MockWindow(processName)
    windows.set(processName, childWindow)
    return { contentWindow: childWindow }
  }
  ...
  const childAPromise = rootProcess.createProcess({})

  const childAProcess = core.ChildProcess(
    {
      ...windowMethods({ source: childWindow, windows }),
      processName
    },
  ...
  childAProcess.addEventListener(`error`, console.error)

  const childBPromise = rootProcess.createProcess({})

  const childBProcess = core.ChildProcess(
    {
      ...windowMethods({ source: childWindow, windows }),
      processName
    },
  ...
  childBProcess.addEventListener(`error`, console.error)

  const [childA, childB] = await Promise.all([childAPromise, childBPromise])
  const abs = await childA()
  const answer = await childB(abs)
